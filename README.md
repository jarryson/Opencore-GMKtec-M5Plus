...
3. Chrome fix by GLfriend ~~running `open /Applications/Chrome.app --args --disable-gpu`, and in `chrome://flags`, disable `#enable-gpu-rasterization` and choose opengl `#use-angle` flags. And chrome seems usable.~~
4. Use GLFriend app to open chrome like apps is another option.
5. Virtualization you can use Parallels 13.1, need `SYSTEM_VERSION_COMPAT=1` environment variable. You can use [this launcher](https://github.com/mikigal/ryzen-hackintosh/blob/master/Resources/Parallels%20Desktop%20Launcher.app.zip). OR replace Paralles Contents/MacOS/inittool with Tools/paralles.
